...
#define TIMESTRING3  "Process %d has %d tickets \n"
...
static uint32 total_num_quanta = 0;

// Used for lottery drawing
static int total_tickets = 0;

// Used to track time for processes' execution
uint32
my_timer_get()
{
    return (total_num_quanta * 100) + (total_num_quanta * 1);
}
...
ProcessSchedule ()
{
  ...
  int         i;
  int         pid;
  int         ticket;   //winning ticket
  int         high = 0; //upper ticket bound
  int         low = 0;  //lower ticket bound
  Link *l;
  ...
  // Increment the total number of quanta
  total_num_quanta++;
    
  // Grab the first process on the runQueue and move it to the back
  ...
  // Draw the winning ticket.  Returns a value between 1 
  // and total_tickets
  ticket = (random() % total_tickets);

  // Grab the link to the first item in the queue.  We like
  // links
  l = (QueueFirst (&runQueue));
  
  // Cycle through the runQueue searching for the winning
  // ticket.  We grab the link's object, then set the high
  // boundary to the low boundary + objects number of tickets.
  // If the ticket is within this range, break out.  Otherwise,
  // keep searching.  Set low to current high and repeat.
  while (l != NULL)
  {
    pcb = (PCB *)(l->object);
    high += (pcb->tickets);
    if (ticket >= low && ticket < high)
        break;
    low = high;
    l = (QueueNext(l));
  }
  
  // Set the currentPCB to the winner
  currentPCB = pcb;
  ...
  while (!QueueEmpty (&zombieQueue)) {
    ...
    // Remove destroyed processes tickets from the pool
    total_tickets = total_tickets - pcb->tickets;
    QueueRemove (&pcb->l);
...
ProcessSuspend (PCB *suspend)
{
  ...
  // Remove the sleeping process's tickets from the pool
  total_tickets = total_tickets - suspend->tickets;
...
ProcessWakeup (PCB *wakeup)
{
  ...
  // Add the awakening process's tickets back to the pool
  total_tickets = total_tickets + wakeup->tickets;
}
...
ProcessFork (VoidFunc func, uint32 param, int p_nice, int p_info,char *name, int isUser)
{
  ...
  //--------------------------------------
  // In traps.c, TrapProcessCreateHandler declares that pnice must
  // be between -20 and 19.  In our case, we do not want a negative
  // p_nice value.  We check if p_nice is less than or equal to zero.
  // If so, we set the process's ticket value to 1.  If p_nice is 
  // greater than 19, we set the process's ticket value to 19.  Otherwise,
  // we set the forked process's tickets value to the supplied p_nice. We
  // then increment total tickets by the amount of p_nice.  We finally set
  // the process's p_info member to the supplied p_info value.
